...
<body>
	...
	<div class="wrapper">
	   ...
	   <!-- Ini bagian atas pojok kanan (navbar profil user) -->
	   	<div class="navbar-collapse collapse">
            ...
            <tr>
			 <th class="align-middle text-center">No</th>
                ...
                <th class="align-middle text-center">Status Peminjaman</th>
                
			</tr>
			<?php
                        $i = 1;
                        $query = mysqli_query($koneksi, "SELECT * FROM tb_peminjaman LEFT JOIN tb_user ON tb_user.id_user = tb_peminjaman.id_user LEFT JOIN tb_buku ON tb_buku.id_buku = tb_peminjaman.id_buku WHERE tb_peminjaman.id_user=" . $_SESSION['tb_user']['id_user']);
                        while ($data = mysqli_fetch_array($query)) {
                        ?>
                            <tr>
                                <td class="align-middle text-center"><?php echo $i++; ?></td>
                                <td class="align-middle text-center"><?php echo $data['nama']; ?></td>
                                <td class="align-middle text-center"><?php echo $data['judul']; ?></td>
								<td class="align-middle text-center"><?php echo $data['kode_peminjaman']; ?></td>
                                <td class="align-middle text-center"><?php echo $data['tanggal_peminjaman']; ?></td>
                                <td class="align-middle text-center"><?php echo $data['tanggal_pengembalian']; ?></td>
		  ...
		  <a href="peminjaman_tambah.php" class="btn btn-primary" ><i class="align-middle" data-feather="shopping-cart"></i>  Tambah Peminjaman</a>
            </div>
        </div>
    </div>
</div>
